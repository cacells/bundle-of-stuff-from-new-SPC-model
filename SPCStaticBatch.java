...
public class SPCStaticBatch extends JFrame implements Runnable {
...
//    Color[] colours = {Color.white,Color.black,Color.green,Color.blue,Color.yellow,Color.red,Color.pink};
	...
	public SPCStaticBatch(int size) {
	...
	public void setOfIterations(int exp,int rep){
		...
		experiment = new SPCGridStatic(64, celltypes-2, frac,false);//new experiment
		...
		avProliferations = (double) SPCCell.totalproliferations/(64.0*64.0);
		...
		for(int i=0; i<celltypes; i++){
			if(SPCCell.cellcounts[i]>0){
				avTypeProliferations = (double)(SPCCell.prolifcounts[i])/(SPCCell.cellcounts[i]*1.0);
			}else{
				avTypeProliferations=0.0;
			}
			avTypes = (double)(SPCCell.cellcounts[i])/(64.0*64.0);
			avStains = (SPCCell.stainsums[i])/(64.0*64.0);
	...
	public void outputData(){
		...
		try{
			...
			BufferedWriter bufProlif = new BufferedWriter(new FileWriter(fileprefix+"ProlifFrac.txt"));//Proliferation.txt
			BufferedWriter bufTypes = new BufferedWriter(new FileWriter(fileprefix+"TypeFrac.txt"));//Types.txt
			BufferedWriter bufTypeProlif = new BufferedWriter(new FileWriter(fileprefix+"ProlifFractionPerType.txt"));//TypeProliferation.txt
			BufferedWriter bufStain = new BufferedWriter(new FileWriter(fileprefix+"AverageStainPerType.txt"));//StainProliferation.txt
			String[] headers = {"Space","SC","SPC1","SPC2","SPC3","SPC4","SPC5"};
			double frac=0.0;
	...
	public static void main(String args[]) {
		...
		if(args.length>0){
			fileprefix=args[0];
			SPCStaticBatch s = new SPCStaticBatch(64);
			s.start();
		}else{
			SPCStaticBatch s = new SPCStaticBatch(64);
			s.start();
